...
namespace idee5.Common.Net46.Plugins {
    // Taken from https://github.com/MefContrib/MefContrib/blob/master/src/MefContrib/Hosting/RecursiveDirectoryCatalog.cs
    /// <summary>
    ...
    public class RecursiveDirectoryCatalog : ComposablePartCatalog, INotifyComposablePartCatalogChanged, ICompositionElement {
        ...
        public RecursiveDirectoryCatalog(string path, string searchPattern, ReflectionContext reflectionContext)
        {
            _path = path;
        ...
        /// Initializes a new instance of the <see cref=""/> class.
        /// </summary>
        /// <param name="path">The plugins path.</param>
        /// <param name="reflectionContext"></param>
        public RecursiveDirectoryCatalog(string path, ReflectionContext reflectionContext) : this(path, searchPattern: "*.dll", reflectionContext: reflectionContext)
        {
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="RecursiveDirectoryCatalog"/> class with <see
        /// cref="ComposablePartDefinition"/> objects based on all the DLL files in the specified
        /// directory path and its sub-directories.
        /// </summary>
        /// <param name="path">Path to the directory to scan for assemblies to add to the catalog.</param>
        public RecursiveDirectoryCatalog(string path) : this(path, searchPattern: "*.dll")
        {
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="RecursiveDirectoryCatalog"/> class with <see
        /// cref="ComposablePartDefinition"/> objects based on the specified search pattern in the
        /// specified directory path path and its sub-directories.
        /// </summary>
        /// <param name="path">Path to the directory to scan for assemblies to add to the catalog.</param>
        ...
        private static IEnumerable<string> GetFoldersRecursive(string path)
        {
            ...
            foreach (var child in Directory.GetDirectories(path)) { result.AddRange(GetFoldersRecursive(child)); }
            return result;
        }

        private void Initialize(string path, string searchPattern)
        {
            IEnumerable<DirectoryCatalog> directoryCatalogs = GetFoldersRecursive(path).Select(dir => new DirectoryCatalog(dir, searchPattern, _reflectionContext));
            ...
            foreach (var catalog in directoryCatalogs) { _aggregateCatalog.Catalogs.Add(catalog); }
        }

        /// <summary>
        /// Gets the part definitions that are contained in the recursive directory catalog.
        /// (Overrides ComposablePartCatalog.Parts.)
        /// </summary>
        ...
        {
            return string.Format(CultureInfo.CurrentCulture, format: "{0} (RecursivePath=\"{1}\")", args: new object[] { new[] { GetType().Name, _path } });
        }
