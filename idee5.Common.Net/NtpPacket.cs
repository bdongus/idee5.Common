...
[assembly: InternalsVisibleTo("idee5.Common.Net.Tests")]
namespace idee5.Common.Net {
    ...
    /// or even better call <see cref="NtpClient.GetCorrectionOffset" />.
    /// </para>
    /// <para>
    ...
    public class NtpPacket {
        ...
        /// </remarks>
        /// <seealso cref="NtpClient.Query(NtpPacket)" />
        public NtpPacket()
            : this(new byte[48])
        {
            Mode = NtpMode.Client;
        ...
        internal void ValidateReply(NtpPacket request)
        {
            ...
            if (Stratum == 0)
                throw new NtpException(this, $"Received Kiss-o'-Death SNTP packet with code 0x{ReferenceId:x}.");
        ...
        private DateTime? GetDateTime64(int offset)
        {
            ...
            if (field == 0)
                return null;
            return new DateTime(_epoch.Ticks + Convert.ToInt64(field * (1.0 / (1L << 32) * 10000000.0)));
        }

        private void SetDateTime64(int offset, DateTime? value)
        { SetUInt64BE(offset, value == null ? 0 : Convert.ToUInt64((value.Value.Ticks - _epoch.Ticks) * (0.0000001 * (1L << 32)))); }
